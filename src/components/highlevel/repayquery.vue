<template>
  <div>
    <Input
      v-model="partnerloanno"
      placeholder="还款的借款编号（账单还款需要输入账单编号， 订单还款输入三方借款编号）"
      style="width: 450px"
    />
...
export default {
  ...
  methods: {
    listJobs(params) {
      ...
      var sql = {
        sql:
          "select * from job where proxy_serial_no in ('" +
          repayProxySerialNo +
          "')"
      };
    ...
    initpaymentstatus(params) {
      ...
      this.axios
        .post(uri, {
          sql:
            "update  user_pay  set payment_confirm_status = 0,payment_confirm_time = null where proxy_serial_no ='" +
    ...
    initaccountreportstatus(params) {
      ...
      this.axios
        .post(uri, {
          sql:
            "update  user_pay  set account_report_status = 0,account_report_time=null where proxy_serial_no ='" +
            params.row.proxy_serial_no +
            "'"
        })
    ...
    */
      ...
      this.axios
        .post(uri, {
          sql:
            "select * from user_pay where partner_loan_no = '" +
            this.partnerloanno +
            "'"
        })
