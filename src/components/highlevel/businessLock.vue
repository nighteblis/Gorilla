...
export default {
  ...
  methods: {
    delrediskey(params) {
      ...
      var uri =
        "http://172.16.2.39:7280/api/redis/" +
        this.$store.state.selectedEnv +
        "";
      console.log(uri);

      this.axios
        .post(uri, { command: "del", arguments: "" + params.row.key + "" })
        .then(response => {
    ...
    handleSubmit(showoriginalalert) {
      ...
      this.axios
        .post(uri, {
          command: "keys",
          arguments: "*BusinessLockService*" + this.userkey + "*"
        })
        .then(response => {
          console.log(response);
